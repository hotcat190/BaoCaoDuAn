#heading(level: 1, numbering: none)[Chương 3: Thiết kế kiến trúc hệ thống]
...
=== Các luồng xác thực, ủy quyền và bảo mật hệ thống

Hệ thống áp dụng kiến trúc bảo mật hỗn hợp (Hybrid Security Architecture), kết hợp song song cơ chế xác thực nội bộ thông qua định danh kết hợp mật khẩu và cơ chế ủy quyền phân tán qua Google OAuth2. Quản lý quyền truy cập được thực hiện theo cơ chế phi trạng thái (Stateless) đối với các API nghiệp vụ thông qua mã thông báo JWT.

#figure(
  image("/images/sequence_diagrams/auth_flows/local_auth.svg"),
  caption: [Sơ đồ tuần tự luồng xác thực nội bộ],
...
#figure(
  ...
  caption: [Sơ đồ tuần tự luồng ủy quyền qua Google OAuth2],
) <fig-oauth2-sequence>

#figure(
  image("/images/sequence_diagrams/auth_flows/rbac.svg"),
  caption: [Sơ đồ tuần tự luồng kiểm soát phân quyền],
) <fig-rbac-sequence> 

=== Các luồng quản lý và truy xuất tài nguyên nội dung
...
=== Các luồng tương tác người dùng
Các luồng tương tác người dùng gồm tìm kiếm, gửi bình luận, đánh giá truyện, lưu lịch sử đọc truyện.

#figure(  
  image("/images/sequence_diagrams/user_interactions/search.svg"),
  caption: [Sơ đồ tuần tự luồng tìm kiếm truyện]
)
#v(60pt)
#figure(  
  image("/images/sequence_diagrams/user_interactions/comment.svg"),
  caption: [Sơ đồ tuần tự luồng gửi bình luận]
)

#figure(  
  image("/images/sequence_diagrams/user_interactions/rate.svg"),
  caption: [Sơ đồ tuần tự luồng đánh giá truyện]
)

#v(30pt)

#figure(  
  image("/images/sequence_diagrams/user_interactions/reading_history.svg"),
  caption: [Sơ đồ tuần tự luồng lưu lịch sử đọc truyện]
)
